...
def nuscenes_data_prep(
    ...
    if load_augmented is None:
        ...
        nuscenes_converter.create_nuscenes_infos(
            root_path, info_prefix, out_dir, version=version, max_sweeps=max_sweeps #! 增加out_dir
        )
    ...
    print('============')
    create_groundtruth_database(  #? 这个函数创造的gt_database在train-pipeline-db_sampler(type: ObjectPaste)中被用到了，主要用于数据增强
        dataset_name,
        root_path,
        info_prefix,
        f"{out_dir}/{info_prefix}_infos_train.pkl",
        load_augmented=load_augmented,
        eval_version='detection_cvpr_2019',  #! 新加
    )
