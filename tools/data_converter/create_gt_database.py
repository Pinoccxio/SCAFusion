...
def create_groundtruth_database(
    ...
    info_path=None, # data/nuscenes/nuscenes_infos_train.pkl
    mask_anno_path=None, 
    used_classes=None,
    ...
    bev_only=False,
    coors_range=None,
    with_mask=False,  # 控制是否加入图像 默认是False
    load_augmented=None,
    eval_version=None,  #! 新加
):
        ...
        if not load_augmented:
            dataset_cfg.update(
                ...
                pipeline=[
                    ...
                    dict(
                        type="LoadPointsFromMultiSweeps",  # 融合多帧lidar
                        sweeps_num=10,
                        use_dim=[0, 1, 2, 3, 4],
    ...
    if osp.exists(database_save_path): #! 新增 避免数据混淆
    ...
    group_counter = 0
    for j in track_iter_progress(list(range(len(dataset)))):  # len(data_infos) 323
        ...
        for i in range(num_obj):
            filename = f"{image_idx}_{names[i]}_{i}.bin"  #? 这里是gt_database中文件的命名规则
            abs_filepath = osp.join(database_save_path, filename)
            rel_filepath = osp.join(f"{info_prefix}_gt_database", filename)

            # save point clouds and image patches for each object
            gt_points = points[point_indices[:, i]] # 获取物体点云
            gt_points[:, :3] -= gt_boxes_3d[i, :3] # 将点云坐标转换到物体局部坐标系下
            ...
            with open(abs_filepath, "w") as f:
                gt_points.tofile(f) # 将点云数据写入bin文件
            ...
            if (used_classes is None) or names[i] in used_classes:
                db_info = {
                    ...
                    "difficulty": difficulty[i],  # 默认为0 nusc为0
                }
                local_group_id = group_ids[i]
                # if local_group_id >= 0:
                if local_group_id not in group_dict: # 如果第一次出现
                    group_dict[local_group_id] = group_counter 
                    group_counter += 1
                db_info["group_id"] = group_dict[local_group_id]
                if "score" in annos:
                    # 确保在全部帧中，每帧的每个物体都有一个唯一的group_id，即使local_group_id为一样
