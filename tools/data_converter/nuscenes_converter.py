...
def create_nuscenes_infos(
    ...
    if version == "v1.0-trainval":
        ...
        train_scenes = splits.sim_train  #! 原本是mini_train mini_val
        val_scenes = splits.sim_val    
    else:
    ...
    if test:
        ...
        # info_path = osp.join(root_path, "{}_infos_test.pkl".format(info_prefix)) #! 修改
        ...
        data = dict(infos=train_nusc_infos, metadata=metadata)
        # info_path = osp.join(root_path, "{}_infos_train.pkl".format(info_prefix)) #! 修改
        info_path = osp.join(out_dir, "{}_infos_train.pkl".format(info_prefix))
        mmcv.dump(data, info_path)
        data["infos"] = val_nusc_infos
        # info_val_path = osp.join(root_path, "{}_infos_val.pkl".format(info_prefix)) #! 修改
...
def get_available_scenes(nusc):
    ...
    for scene in nusc.scene:  #? 测试场景是否真实可用
...
def _fill_trainval_infos(nusc, train_scenes, val_scenes, test=False, max_sweeps=10):
    ...
    for sample in mmcv.track_iter_progress(nusc.sample):
        ...
        lidar_path, boxes, _ = nusc.get_sample_data(lidar_token) #! 这一步会从category里获得类别
        ...
        l2e_r_mat = Quaternion(l2e_r).rotation_matrix # 将四元数转换为旋转矩阵 给obtain_sensor2top用
        ...
        while len(sweeps) < max_sweeps:
            if not sd_rec["prev"] == "": # 如果存在前一帧
                ...
                break
        info["sweeps"] = sweeps   # 每个sample对应10帧sample_data 最后一帧是上一次的sample，其余是sweep
        ...
        if not test:
            ...
            valid_flag = np.array(
                [
                    (anno["num_lidar_pts"] + anno["num_radar_pts"]) > 0 # 相加>0为valid
            ...
            gt_boxes = np.concatenate([locs, dims, -rots - np.pi / 2], axis=1) # LiDAR坐标系下
