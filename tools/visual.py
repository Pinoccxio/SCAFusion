...
#* v4 一路相机改为多路相机，并将img和lidar的可视化解耦，添加gt/pred/gt_pred的split参数
#* v5 增加了对lidar端bbox范围越界的检查
...
def visualize_misdetections(data_loader=None, outputs=None, 
                            output_dir="mis_match_vis", 
                            threshold=0.5, with_text=True,
                            xlim=None, ylim=None, split="gt_pred"):
    os.makedirs(output_dir, exist_ok=True)
    colors = {
        'gt': (0, 255, 0),  # Green For GT
    ...
    for idx, data in tqdm(enumerate(data_loader), total=len(data_loader)): 
        ...
        for k, img_path in enumerate(metas['filename']):
            ...
            if img is None:
                print(f'Could not read image from {img_path}, skipping now...')
                continue
            lidar2img = metas['lidar2image'][k]
            vis_img = img.copy()
            if split == "gt" or split == "gt_pred":
            ...
            if split == "pred" or split == "gt_pred":
                for m, pred_idx in enumerate(pred_indices):
                    pred_box = pred_boxes[pred_idx:pred_idx+1]
                    pred_label = pred_labels[pred_idx].item()
                    pred_score = pred_scores[pred_idx].item()
                    pred_iou = pred_ious[m]
                    pred_corners = project_box(pred_box, lidar2img, img.shape[:2])
                    if not pred_corners:
                        continue
                    text = f"{pred_label}:{pred_score:.2f}:{pred_iou:.2f}" if with_text else ""
                    draw_boxes(vis_img, pred_corners, colors['pred'], text)
                for m, mis_idx in enumerate(mis_indices):
            ...
            # Save Img
            img_path = os.path.join(output_dir,f'camera-{k}')
            os.makedirs(img_path, exist_ok=True)
            save_path = os.path.join(img_path, f"frame{idx:04d}.png")
            cv2.imwrite(save_path, vis_img)
        # <<< Img Visualization <<<
        ...
        if split == "gt" or split == "gt_pred":
            for gt_idx in gt_indices:
                ...
                if coords[:,0].max() > xlim[1] or coords[:,0].min() < xlim[0] or \
                    coords[:,1].max() > ylim[1] or coords[:,1].min() < ylim[0]:
                    continue
                plt.plot(
                    coords[:,0],
                    coords[:,1],
        ...
        save_path = os.path.join(lidar_path, f"frame{idx:04d}.png")
        fig.savefig(
            save_path,
            dpi=10,
            facecolor="black",
...
def parse_args():
    ...
    parser.add_argument("--split", type=str, default="gt/pred/gt_pred", help="split")
...
def main():
    ...
    if rank == 0:
        ...
        visualize_misdetections(data_loader, outputs, 
                                ...
                                xlim=xlim, ylim=ylim, split=args.split)
