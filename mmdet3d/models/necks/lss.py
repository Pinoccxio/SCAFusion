...
class LSSFPN(nn.Module):
    ...
    def forward(self, x: List[torch.Tensor]) -> torch.Tensor:
        x1 = x[self.in_indices[0]] # B,512,45,45
        assert x1.shape[1] == self.in_channels[0]

        x2 = x[self.in_indices[1]] # B,128,90,90
        ...
        x = torch.cat([x1, x2], dim=1) # B,(512+128),90,90

        x = self.fuse(x) # B,256,90,90
        if self.scale_factor > 1:
            x = self.upsample(x) # B,256,180,180
        return x
