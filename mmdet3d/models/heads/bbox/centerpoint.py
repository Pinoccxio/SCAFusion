...
class CenterHead(BaseModule):
    ...
    def __init__(
        ...
        for num_cls in num_classes: # task类
            heads = copy.deepcopy(common_heads)
            heads.update(dict(heatmap=(num_cls, num_heatmap_convs))) # heatmap, cls, reg, height, dim, rot, vel
    ...
    def forward_single(self, x):
        ...
        Args:
            x (torch.Tensor): Input feature map with the shape of
                [B, 512, 128, 128]. [B, 256, 180, 180]
        Returns:
            list[dict]: Output results for tasks.
        """
        ret_dicts = []

        x = self.shared_conv(x) # (B, 64, 180, 180)
