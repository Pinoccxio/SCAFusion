...
#? data['img'] data['img_aug_matrix']  是否is_train 结果不同
@PIPELINES.register_module()
class ImageAug3D:
    def __init__(
        self, final_dim, resize_lim, bot_pct_lim, rot_lim, rand_flip, is_train
    ):
        self.final_dim = final_dim # [256, 704]  # 最终尺寸
        self.resize_lim = resize_lim # [0.38, 0.55]
        self.bot_pct_lim = bot_pct_lim # [0.0 ,0.0]  # 底部裁剪比例范围
        self.rand_flip = rand_flip # true
        self.rot_lim = rot_lim # [-5.4, 5.4]  
    ...
    def sample_augmentation(self, results):
        ...
        if self.is_train:
            resize = np.random.uniform(*self.resize_lim) # 生成一个介于0.38和0.55之间的标量
            resize_dims = (int(W * resize), int(H * resize)) 
            newW, newH = resize_dims
            crop_h = int((1 - np.random.uniform(*self.bot_pct_lim)) * newH) - fH # 垂直方向的裁减起点
            crop_w = int(np.random.uniform(0, max(0, newW - fW))) # 水平方向的裁减起点
            ...
            crop = (crop_w, crop_h, crop_w + fW, crop_h + fH) # (0, 512, 704, 768)  # 裁减左上角 右下角
            flip = False
            rotate = 0
    ...
    def img_transform(
        ...
        # adjust image
        img = img.resize(resize_dims) # 缩放
        img = img.crop(crop) # 裁减
        if flip:
            img = img.transpose(method=Image.FLIP_LEFT_RIGHT) # 翻转
        img = img.rotate(rotate) # 旋转 expand默认false
    ...
    def __call__(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for img in imgs: # 每个img不同
            ...
            transform[:2, :2] = rotation # 2D旋转矩阵
            transform[:2, 3] = translation # 2D平移向量
            new_imgs.append(new_img)
            transforms.append(transform.numpy())
        data["img"] = new_imgs
        # update the calibration matrices
        data["img_aug_matrix"] = transforms # 记录了从原始图像坐标到新图像坐标的变换矩阵
        return data


#? data['points'] data['gt_bboxes_3d'] data['lidar_aug_matrix'] 是否is_train 结果不同
@PIPELINES.register_module()
class GlobalRotScaleTrans:
    def __init__(self, resize_lim, rot_lim, trans_lim, is_train):
        self.resize_lim = resize_lim # [0.9, 1,1]
        self.rot_lim = rot_lim # [-0.7854, 0.7854]
        self.trans_lim = trans_lim # 0.5
        self.is_train = is_train # true
...
class GridMask:
    ...
    def __init__(
        ...
        self.rotate = rotate # 1 可以旋转的角度范围
        self.offset = offset # false 是否应用遮罩偏移
        self.ratio = ratio # 0.5 # 遮罩占网格的比例 
        self.mode = mode # 1 # 0 保留原图，遮罩置0    1 反之
        self.st_prob = prob # 0.0 # 初始应用概率
        self.prob = prob # 0.0 # 当前应用概率
        self.epoch = None 
        self.max_epoch = max_epoch # 6 
        self.fixed_prob = fixed_prob # true # 是否固定概率
    ...
    def set_prob(self, epoch, max_epoch): # 随着epoch增加，概率增加
        self.prob = self.st_prob * self.epoch / self.max_epoch

    def __call__(self, results):
        # 1. 根据概率决定是否应用GridMask
        if np.random.rand() > self.prob:
            return results

        # 2. 获取输入图像及其尺寸
        imgs = results["img"]
        h = imgs[0].shape[0]  # 图像高度
        w = imgs[0].shape[1]  # 图像宽度

        # 3. 设置网格参数
        self.d1 = 2  # 网格最小尺寸
        self.d2 = min(h, w)  # 网格最大尺寸
        hh = int(1.5 * h)  # 扩大的高度,用于旋转
        ww = int(1.5 * w)  # 扩大的宽度,用于旋转
        
        # 4. 随机生成网格大小和遮罩长度 (遮罩在网格划分内)
        d = np.random.randint(self.d1, self.d2)  # 随机网格大小
        if self.ratio == 1:
            self.l = np.random.randint(1, d)  # 随机遮罩长度
        else:
            self.l = min(max(int(d * self.ratio + 0.5), 1), d - 1)  # 根据ratio计算遮罩长度
            
        # 5. 创建遮罩
        mask = np.ones((hh, ww), np.float32)  # 初始化遮罩全为1
        st_h = np.random.randint(d)  # 随机起始高度偏移
        st_w = np.random.randint(d)  # 随机起始宽度偏移

        # 6. 在高度方向创建网格遮罩
        if self.use_h:
            for i in range(hh // d):
                s = d * i + st_h  # 遮罩起始位置
                t = min(s + self.l, hh)  # 遮罩结束位置
                mask[s:t, :] *= 0  # 将遮罩区域置0
        ...
        if self.use_w:
            for i in range(ww // d):
                s = d * i + st_w  # 遮罩起始位置
                t = min(s + self.l, ww)  # 遮罩结束位置
                mask[:, s:t] *= 0  # 将遮罩区域置0

        # 8. 旋转遮罩
        r = np.random.randint(self.rotate)  # 随机旋转角度
        mask = Image.fromarray(np.uint8(mask))  # 转换为PIL图像
        mask = mask.rotate(r)  # 旋转
        mask = np.asarray(mask)  # 转回numpy数组

        # 9. 裁剪回原始尺寸
        mask = mask[
            (hh - h) // 2 : (hh - h) // 2 + h,  # 高度裁剪
            (ww - w) // 2 : (ww - w) // 2 + w   # 宽度裁剪
        ]

        # 10. 处理遮罩格式
        mask = mask.astype(np.float32)
        mask = mask[:, :, None]  # 增加通道维度
        if self.mode == 1:
            mask = 1 - mask  # 反转遮罩

        # 11. 应用遮罩到图像
        if self.offset:  # 如果使用偏移
            offset = torch.from_numpy(2 * (np.random.rand(h, w) - 0.5)).float()  # 随机偏移值
            offset = (1 - mask) * offset  # 只在遮罩区域应用偏移
            imgs = [x * mask + offset for x in imgs]  # 应用遮罩和偏移
        else:
            imgs = [x * mask for x in imgs]  # 只应用遮罩
...
#? data['points'] data['gt_bboxes_3d'] data['gt_masks_bev'] data['lidar_aug_matrix']
@PIPELINES.register_module()
class RandomFlip3D:
    def __call__(self, data: Dict[str, Any]) -> Dict[str, Any]:
...
#? data['points'] data['gt_bboxes_3d'] data['gt_labels_3d']
@PIPELINES.register_module()
class ObjectPaste:
    ...
    def __init__(self, db_sampler, sample_2d=False, stop_epoch=None):
        ...
        self.db_sampler = build_from_cfg(db_sampler, OBJECTSAMPLERS) # 在dbsampler.py中注册
        self.epoch = -1
        self.stop_epoch = stop_epoch # -1  禁用GT增强

    def set_epoch(self, epoch): # 通过custom_3d.py中的set_epoch()传递给pipeline的每个拥有set_epoch()的transform
        self.epoch = epoch

    @staticmethod # 移除原points中在sampled_gt_bboxes_3d中的点
    def remove_points_in_boxes(points, boxes):
        ...
        points = points[np.logical_not(masks.any(-1))] # 返回points中不在boxes中的点
        return points
    ...
    def __call__(self, data):
        ...
        Returns:
            dict: Results after object sampling augmentation, \
                ...
                in the result dict.
        """
        if self.stop_epoch is not None and self.epoch >= self.stop_epoch: # 停止使用GT增强
            return data # 当stop_epoch为-1时，不使用GT增强
        gt_bboxes_3d = data["gt_bboxes_3d"]
        gt_labels_3d = data["gt_labels_3d"]

        # change to float for blending operation
        points = data["points"]
        if self.sample_2d: # 是否对图像也进行增强  默认False
            img = data["img"]
        ...
        if sampled_dict is not None:
            ...
            points = self.remove_points_in_boxes(points, sampled_gt_bboxes_3d) # 删除points中在sampled_gt_bboxes_3d中的点 避免干扰
...
#? data['gt_bboxes_3d'] data['gt_labels_3d']  其中还对bbox的yaw进行了limit rad to [-pi, pi]
@PIPELINES.register_module()
class ObjectRangeFilter:
    ...
    def __init__(self, point_cloud_range): #  [-51.2, -51.2, -5.0, 51.2, 51.2, 3.0]
...
#? data['gt_bboxes_3d'] data['gt_labels_3d']
@PIPELINES.register_module()
