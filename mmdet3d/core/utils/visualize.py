...
def visualize_camera(
    ...
    *,
    ...
    thickness: float = 1,  # 默认4
) -> None:
    canvas_ori = image.copy()
    canvas = image.copy()
    ...
    #! 绘制原始图像
    fpath_ori = fpath.replace('.png', '_ori.png')
    mmcv.imwrite(canvas_ori, fpath_ori)
...
def visualize_lidar(
    ...
    *,
        ...
        for index in range(coords.shape[0]):
            ...
            if score != 1.0:
                ...
                plt.plot(
                    ...
                    coords[index, :, 1],
                    linewidth=thickness,
                    color=np.array([144, 238, 144]) / 255,
                )
    ...
    #! 绘制原始点云
    fig_ori = plt.figure(figsize=(xlim[1] - xlim[0], ylim[1] - ylim[0]))
    ax = plt.gca()
    ax.set_xlim(*xlim)
    ax.set_ylim(*ylim)
    ax.set_aspect(1)
