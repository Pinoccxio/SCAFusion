...
class HungarianAssigner3D(BaseAssigner):
    ...
    def assign(self, bboxes, gt_bboxes, gt_labels, cls_pred, train_cfg):
        ...
        if torch.isnan(cost).any() or torch.isinf(cost).any():
            print(f"[警告] 检测到无效值:")
            print(f"NaN数量: {torch.isnan(cost).sum().item()}")
            print(f"Inf数量: {torch.isinf(cost).sum().item()}")
            print(f"cls_cost 无效值: NaN={torch.isnan(cls_cost).sum().item()}, Inf={torch.isinf(cls_cost).sum().item()}")
            print(f"reg_cost 无效值: NaN={torch.isnan(reg_cost).sum().item()}, Inf={torch.isinf(reg_cost).sum().item()}")
            print(f"iou_cost 无效值: NaN={torch.isnan(iou_cost).sum().item()}, Inf={torch.isinf(iou_cost).sum().item()}")
